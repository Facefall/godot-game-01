...
func _unhandled_input(event: InputEvent) -> void:
	if event.is_action_pressed("remove_dirt"):
		...
		if ToolManager.selected_tool == DataTypes.Tools.PlantCorns or ToolManager.selected_tool == DataTypes.Tools.PlantTomato:
			get_cell_under_mouse()
			add_crop()
...
func add_crop() -> void:
	if distance < 20:
		if ToolManager.selected_tool == DataTypes.Tools.PlantCorns:
